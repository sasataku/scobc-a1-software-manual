...
:zephyrlgetting-started: https://docs.zephyrproject.org/latest/develop/getting_started/index.html
...
[TIP]
====
According to {zephyr-install-dependencies}[the upstream Zephyr Install dependencies], flashing using
WSL is not supported. However, we have confirmed that flashing is possible by enabling USB devices
connected to WSL2 using the {wsl-usb-connect}[Microsoft’s USB connection guide].

The following USB device need to be attached to WSL2:

* 0403:6011 USB Serial Converter A, USB Serial Converter B, USB Seria...
====
